...
async function makeTencentCloudCall(params: {
  ...
  try {
    const client = new CccClient({
      ...
      region: 'ap-guangzhou', // Or your CCC region
      profile: {
        httpProfile: {
    ...
    const apiParams = {
      ...
      Callees: [params.to], // Ensure it's an array
      Callers: [params.callerNumber], // Ensure it's an array
      IvrId: params.ivrId,
...
serve(async (_req) => {
  ...
  try {
    ...
    let tencentSdkAppId: number | null = null;
    let tencentIvrId: number | null = null;
    ...
    const { data: settingsData, error: settingsError } = await supabase
      ...
      .select('*')
      .eq('id', 1) // Assuming global settings are stored with id = 1
    ...
    if (!shouldExecute) {
      ...
      return new Response(JSON.stringify({ message: 'Skipped execution due to frequency limit.' }), {
    ...
    if (!accounts || accounts.length === 0) {
      ...
      return new Response(JSON.stringify({ message: 'No accounts to monitor.' }), {
        headers: { 'Content-Type': 'application/json' },
        status: 200,
      });
    }
    ...
    for (const account of accounts) {
      ...
      try {
        ...
        const timelineParams = {
          ...
          'max_results': 5, // Minimum allowed value for v2 user timeline
          since_id: undefined as (string | undefined) // Initialize since_id as undefined
        };
          ...
          newTweetsForThisAccount = timeline.data.data.map((tweet) => {
            ...
            if (tweet.attachments?.media_keys && timeline.includes?.media) {
              mediaUrls = tweet.attachments.media_keys
                .map((key) => timeline.includes!.media!.find((m) => m.media_key === key))
                .filter((media): media is NonNullable<typeof media> => !!media) // Type guard
                .map((media) => media.url || media.preview_image_url) // Prefer full URL
                .filter((url): url is string => !!url); // Filter out undefined URLs
            }
            return {
          ...
          if (newTweetsForThisAccount.length > 0) {
            ...
            if (insertTweetsError) {
              ...
              if ((isTwilioEnabled || isTencentCloudEnabled) && wasIncrementalUpdate && account.user_id) {
                ...
                const { data: usersForCalls, error: usersFetchError } = await supabase
                  .from('user_preferences')
                  .select('user_id, phone_number') // Select user_id as well
                  .eq('phone_notifications_enabled', true)
                ...
                if (usersFetchError) {
                  ...
                  for (const user of usersForCalls) {
                     ...
                     let callProvider: 'twilio' | 'tencent' | null = null;
                     let callIdentifier: string | number | null = null;
                     ...
                     if (callProvider && callIdentifier !== null) {
                       await logCallToDatabase({
                         ...
                         message: tweetForNotification, // Log the sent content
                         provider: callProvider,
                         callSidOrTaskId: String(callIdentifier), // Convert TaskId number to string
                         status: 'initiated', // Or 'queued' depending on API response meaning
                       ...
                       // Log failed initiation attempt
                        await logCallToDatabase({
                            ...
                            provider: callProvider,
                            callSidOrTaskId: null, // No ID generated
      ...
      if (latestTweetIdFetched !== initialLastTweetId || fetchedTweetsCount > 0) {
        ...
        const { error: updateCheckTimeError } = await supabase
            .from('monitored_accounts')
            .update({ last_checked_at: invocationTime.toISOString() })
            .eq('id', account.id);
    ...
    const { error: updateSettingsError } = await supabase
      .from('monitoring_settings')
      .update({ last_execution_time: invocationTime.toISOString() })
      .eq('id', 1);
    ...
    return new Response(JSON.stringify({ success: true, newTweets: totalNewTweets }), {
      headers: { 'Content-Type': 'application/json' },
      status: 200,
    });
  } catch (error) {
