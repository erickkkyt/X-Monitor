...
*   **问题简述**: 在多个服务器端文件（API 路由、页面组件）中调用 `supabase.auth.getUser()` 或其他 Supabase 方法时，出现 `Property 'auth' does not exist on type 'Promise<SupabaseClient<...>>'` 错误。
...
*   **解决方法**: 在所有调用 `createClient()` 的地方（例如 `const supabase = createClient();`）添加 `await`，修改为 `const supabase = await createClient();`。同时确保调用 `createClient()` 的函数本身是 `async` 的。受影响的文件包括 `/api/twitter/accounts/route.ts`, `/api/twitter/add-account/route.ts`, `/api/twitter/delete-account/route.ts`, `/auth/confirm/route.ts`, `/dashboard/accounts/page.tsx`, `/dashboard/page.tsx`。
